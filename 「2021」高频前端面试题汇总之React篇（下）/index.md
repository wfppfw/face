...
- 为了解决状态管理混乱，无法有效同步的问题统一维护管理应用状态;
  - 某一状态只有一个可信数据来源（通·																																																																																																																																																																																																																																													常命名为store，指状态容器）;
...
-  Derivation（衍生）∶ 从应用状态中派生而出，且没有任何其他影响的数据43
...
**总结：**useState 返回的是 array 而不是 object 的原因就是为了**降低使用的复杂度**，返回数组的话可以直接根据顺序解构，而返回对象的话要想使用多次就需要定义别名了。
 ...
 ### useState 第二个函数为什么是异步的

setState会触发UI更新（重新render，执行函数组件件)

由于UI更新是异步任务，所以setState也是一个异步过程

解决方法(再写一个副作用Hook(useEffect)监听变化()
...
- ZSuseEffect 接受包含命令式，可能有副作用代码的函数。
